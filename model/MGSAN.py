...
class GSA_block(nn.Module):
    def __init__(self, in_channels, out_channels, A, heads=3, timedim=64):
        ...
        for i in range(self.heads):
            self.to_q.append(
                nn.Sequential(nn.Conv2d(in_channels, dim_head, kernel_size=(1, 1), padding=(0, 0), stride=(1, 1)),
                              nn.BatchNorm2d(dim_head),
                              nn.ReLU(inplace=True),
                              nn.Conv2d(dim_head, dim_head, kernel_size=(timedim, 1), padding=(0, 0), stride=(1, 1),
                                        groups=dim_head),
                              ))
        ...
        self.to_att = nn.ModuleList()
        for i in range(self.heads):
            self.to_att.append(nn.Conv2d(dim_head, out_channels, kernel_size=(1, 1), stride=(1, 1)))
    ...
    def forward(self, x):
        ...
        for i in range(self.heads):
            ...
            k = self.to_k[i](x)
            v = self.to_v[i](x)
            dots = self.soft(einsum('b c t i, b c t j -> b c i j', q, k))
            dots = self.to_att[i](dots) * self.alpha + self.PA[i].unsqueeze(0).unsqueeze(0)
            out = einsum('b c i j, b c t j -> b c t i', dots, v)
